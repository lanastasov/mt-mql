                ...
                const int &spread[])
{
   ...
   for (int i = limit; i > 0; i--)
   {
      ...
      else if ((Close[i] < Close[i + 4]) && (Setup_Buy > 0) && (Setup_Buy < 9))
      {
         ...
         if (Setup_Buy == 9)
         {
            if (Countdown_Sell > 0)
            {
               ...
               RemoveCount(COUNT_TYPE_SELL_COUNTDOWN, Time[i]);
            }
      ...
      {
         RemoveCount(COUNT_TYPE_BUY_SETUP, Setup_Buy_First_Candle, Setup_Buy); // Remove number objects for this Buy Setup.
         Setup_Buy = 0;
         Setup_Buy_First_Candle = 0;
      ...
      if (Countdown_Buy > 0) // Have a Buy Countdown that can be interrupted.
      {
         ...
         {
            Countdown_Buy = 0;
            RemoveCount(COUNT_TYPE_BUY_COUNTDOWN, Time[i]);
            Setup_Buy = 0;
         }
      }
      ...
      {
         RemoveCount(COUNT_TYPE_SELL_SETUP, Setup_Sell_First_Candle, Setup_Sell); // Remove number objects for this Sell Setup.
         Setup_Sell = 0;
         Setup_Sell_First_Candle = 0;
...
void OnChartEvent(const int id,
                  ...
                  const string &sparam)
{
   if ((id == CHARTEVENT_CHART_CHANGE) || (id == CHARTEVENT_CLICK)) RedrawVisibleLabels();
}
...
// Remove number objects for a given setup or countdown.
void RemoveCount(const ENUM_COUNT_TYPE count_type, const datetime begin, const int n = 0)
{
   ...
   else if ((count_type == COUNT_TYPE_BUY_COUNTDOWN) || (count_type == COUNT_TYPE_SELL_COUNTDOWN))
   {
      ...
      for (int i = begin_candle; i < Bars; i++)
      {
         string name = name_start + IntegerToString(Time[i]);
...
void DoAlert(int i, ENUM_ALERT_TYPE alert_type)
{
	...
	switch(alert_type)
	{
	   ...
	   main_text += " Buy Setup completed by " + TimeToString(Time[i]) + " candle.";
      email_subject += " Buy Setup";
	   break;
	   case ALERT_TYPE_SETUP_SELL:
	   main_text += " Sell Setup completed by " + TimeToString(Time[i]) + " candle.";
      email_subject += " Sell Setup";
	   break;
	   case ALERT_TYPE_PERFECTING_BUY:
	   main_text += " Buy Setup Perfected by " + TimeToString(Time[i]) + " candle.";
      email_subject += " Buy Perfecting";
	   break;
	   case ALERT_TYPE_PERFECTING_SELL:
	   main_text += " Sell Setup Perfected by " + TimeToString(Time[i]) + " candle.";
      email_subject += " Sell Perfecting";
	   break;
	   case ALERT_TYPE_COUNT13_BUY:
	   main_text += " Buy Countdown completed by " + TimeToString(Time[i]) + " candle.";
      email_subject += " Buy Countdown";
	   break;
	   case ALERT_TYPE_COUNT13_SELL:
	   main_text += " Sell Countdown completed by " + TimeToString(Time[i]) + " candle.";
      email_subject += " Sell Countdown";
	   break;
	   case ALERT_TYPE_RESISTANCE:
	   main_text += " TDST Resistance broken by " + TimeToString(Time[i]) + " candle.";
      email_subject += " Buy TDST Resistance";
	   break;
	   case ALERT_TYPE_SUPPORT:
	   main_text += " TDST Support broken by " + TimeToString(Time[i]) + " candle.";
